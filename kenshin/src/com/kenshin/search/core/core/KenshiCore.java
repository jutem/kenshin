...
import com.kenshin.search.core.search.RamSearcher;
import com.kenshin.search.core.search.SegSearcher;
import com.kenshin.search.core.search.manager.SearcherManager;
...
//	private static final List<Directory> bufferB = new ArrayList<Directory>();
	...
	private static final int MAX_INDEXER = 100; //启动索引者数量
	private static final ExecutorService indexerPool = Executors.newFixedThreadPool(MAX_INDEXER);
	
	private static final int DATAPOOLNUM = 100; //每个生产者持有的数据
	private static final int MAX_PRODUCER = 100; //启动生产者数量
	private static final ExecutorService producerPool = Executors.newFixedThreadPool(MAX_PRODUCER);
	
	private static final int PER_MAX_SEARCHER = 1; //每秒搜索者数量
	...
	public static void main(String[] args) throws InterruptedException, IOException {
		...
		for(int i = 0; i < MAX_INDEXER; i++) {
			Indexer indexer;
			try {
				indexer = new Indexer("IndexerNo_" + i, analyzer, data, SEG_PATH);
